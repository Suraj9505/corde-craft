...
import SnippersPageSkeleton from "./_components/SnippersPageSkeleton";
    ...
    if (snippets === undefined) {
        return (
            <div className="min-h-screen">
                <NavigationHeader />
                <SnippersPageSkeleton />
            </div>
        )
    }

    const languages = [...new Set(snippets.map((s) => s.language))];
    // currently popular languages are selected using slice try to implement a logic where the popular languages are the one which has most snippets
    const popularLanguages = languages.slice(0, 5);

    const filteredSnippets = snippets.filter((snippet) => {
        const matchedSearch = snippet.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
            snippet.language.toLowerCase().includes(searchQuery.toLowerCase()) ||
            snippet.userName.toLowerCase().includes(searchQuery.toLowerCase());

        const matchedLanguage = !selectedLanguage || snippet.language === selectedLanguage;

        return matchedSearch && matchedLanguage;
    })
        ...
        <div className="min-h-screen bg-[#0a0a0f]">
            ...
            <div className="relative max-w-7xl mx-auto px-4 py-12">
             ...
             from-blue-500/10 to-purple-500/10 text-sm text-gray-400 mb-6"
                ...
                {/* Snippet Grid */}
                <motion.div
                    className={`grid gap-6 ${view === "grid" ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"
                        : "grid-cols-1 max-w-3xl mx-auto"}`} layout>
